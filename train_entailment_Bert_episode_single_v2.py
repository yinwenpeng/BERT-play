        ...
        if args.do_train:
            ...
            for _ in trange(int(args.num_train_epochs), desc="Epoch"):
                ...
                for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                    ...
                    target_batch_index = random.randint(0,int(len(train_examples)/args.train_batch_size))
                    for target_size, target_batch in enumerate(train_dataloader):#tqdm(train_dataloader, desc="episode")):
                    ...
                    label_1_rep = torch.mv(torch.t(target_reps), target_labels.float())/(1e-6+torch.sum(target_labels.float())) #hidden_size
                    opposite_label_ids = (1-target_labels).float()
                    label_0_rep = torch.mv(torch.t(target_reps), opposite_label_ids)/(1e-6+torch.sum(opposite_label_ids)) #hidden_size
                    ...
                    episode_loss = loss_fct(dot_matrix.view(-1, num_labels), label_ids.view(-1))
                    loss+=episode_loss
