...
def main():
    ...
    target_train_pos_few_dataloader = examples_2_dataloader(train_pos_examples_sequence[-1], sample_train_size)
    target_train_neg_few_dataloader = examples_2_dataloader(train_neg_examples_sequence[-1], sample_train_size)
    ...
    while True:
        ...
        for i in range(len(inter_task_names)-1):
            ...
            pos_label_proptype_emb_cross_tasks.append(torch.mean(target_batch_reps, dim=0, keepdim =True))
            '''
            neg 10
            '''
            ...
            neg_label_proptype_emb_cross_tasks.append(torch.mean(target_batch_reps, dim=0, keepdim =True))
        ...
        loss=0.0
        for i in range(len(inter_task_names)-1):
            composed_labels_proptype_emb = torch.cat((pos_label_proptype_emb_cross_tasks[i], neg_label_proptype_emb_cross_tasks[i]), dim=0)#(2,hidden size)
            '''
            pos 50
            '''
        ...
        composed_labels_proptype_emb = torch.cat((target_composed_pos_label_proptype_emb, target_composed_neg_label_proptype_emb), dim=0)#(2,hidden size)
        # print('composed_labels_proptype_emb:',composed_labels_proptype_emb[:,:20])
        '''replace the composed label emb'''
        # composed_labels_proptype_emb = target_composed_labels_proptype_emb
